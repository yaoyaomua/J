...
enum TokenKind {
    ...
    // Reserved words
    ABSTRACT("abstract"), BOOLEAN("boolean"), BREAK("break"),BYTE("byte"),CASE("case"), CATCH("catch"),
    CONST("const"),CHAR("char"), CLASS("class"), CONTINUE("continue"), 
    DEFAULT("default"), DO("do"), DOUBLE("double"),
    ELSE("else"), EXTENDS("extends"),FINALLY("finally"), FLOAT ("float"), FOR("for"), GOTO("goto"), IF("if"), IMPORT("import"), 
    INSTANCEOF("instanceof"), IMPLEMENTS("implements"),INT("int"), INTERFACE("interface"), NATIVE("native"), NEW("new"), PACKAGE("package"), 
    PRIVATE("private"), PROTECTED("protected"), PUBLIC("public"), 
    RETURN("return"), STATIC("static"), SUPER("super"), SHORT("short"),STRICTFP("strictfp"),SWITCH("switch"), SYNCHRONIZED("synchronized"),THIS("this"), 
    ...
    LE("<="), LNOT("!"), MINUS("-"), PLUS("+"), PLUS_ASSIGN("+="), STAR("*"),DIV("/") ,REM("%"),QUESTION("?"), MINUS_ASSIGN("-="),RIGHT_SHIFT("<<"),LT("<"),
    LSE("<<="),STAR_ASSIGN("*="),NOT_EQUAL("!="),COMPLEMENT("~"),REM_ASSIGN("%="),XOR("^"),XOR_ASSIGN("^="),DIV_ASSIGN("/="),LEFT_SHIFT(">>"),LS_ASSIGN (">>="),LS_ZERO(">>>"),LSZERO_ASSIGN(">>>="),
    GT_EQUAL(">="),AND("&"),AND_ASSIGN("&="),BITWISE_OR("|"),OR_ASSIGN("|="),LOGICAL_OR("||"),
